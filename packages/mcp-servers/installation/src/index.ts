...
// Types for our installation tools
interface InstallPackageParams {
  package: string;
  version?: string;
  options?: string[];
  manager?: 'auto' | 'apt' | 'yum' | 'dnf' | 'pacman' | 'brew' | 'snap';
}

interface InstallOracleParams {
  version?: '21c' | '19c' | '23c';
  memory_gb?: number;
  storage_gb?: number;
  auto_start?: boolean;
...
interface SetupWebServerParams {
  server_type: 'nginx' | 'apache' | 'both';
  ssl_enabled?: boolean;
  domain?: string;
  auto_start?: boolean;
}
...
class InstallationServer {
      ...
      try {
        ...
        switch (name) {
          case 'install_package':
            return await this.installPackage(args as InstallPackageParams);
          
          case 'install_oracle_database':
            return await this.installOracleDatabase(args as InstallOracleParams);
          
          case 'setup_web_server':
            return await this.setupWebServer(args as SetupWebServerParams);
          
          case 'check_system_requirements':
            return await this.checkSystemRequirements(args as any);
  ...
  private async installPackage(params: InstallPackageParams) {
    const { package: pkg, version, options = [], manager = 'auto' } = params;
    
    let packageManager: string = manager;
    if (manager === 'auto') {
    ...
    switch (packageManager) {
      case 'apt':
        command = 'sudo';
        args = ['apt', 'install', '-y'];
        if (version) args.push(`${pkg}=${version}`);
        else args.push(pkg);
        break;
  ...
  private async installOracleDatabase(params: InstallOracleParams) {
    const { 
      version = '21c', 
      memory_gb = 8, 
      storage_gb = 50, 
  ...
  // Web server setup
  private async setupWebServer(params: SetupWebServerParams) {
  ...
  // System requirements check
  private async checkSystemRequirements(params: any) {
    ...
    if (detailed) {
      try {
        ...
        output += `- Uptime: ${systemInfo.uptime} hours\n`;
                 output += `- Load Average: ${os.loadavg().map((load: number) => load.toFixed(2)).join(', ')}\n`;
  ...
  private async getDiskSpace(): Promise<string> {
    try {
  ...
  private async dryRunCommand(command: string, args: string[]): Promise<string> {
