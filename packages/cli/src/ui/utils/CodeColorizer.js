...
function renderHastNode(node, theme, inheritedColor) {
    ...
    if (node.type === 'element') {
        ...
        const children = node.children?.map((child, index) => (<React.Fragment key={index}>
    ...
    if (node.type === 'root') {
        ...
        return node.children?.map((child, index) => (<React.Fragment key={index}>
...
export function colorizeCode(code, language, availableHeight, maxWidth) {
    ...
    try {
        ...
        return (<MaxSizedBox maxHeight={availableHeight} maxWidth={maxWidth} additionalHiddenLinesCount={hiddenLinesCount} overflowDirection="top">
        ...
        const padWidth = String(lines.length).length; // Calculate padding width based on number of lines
        return (<MaxSizedBox maxHeight={availableHeight} maxWidth={maxWidth} overflowDirection="top">
