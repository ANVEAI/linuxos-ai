...
export const TableRenderer = ({ headers, rows, terminalWidth, }) => {
    ...
    const renderCell = (content, width, isHeader = false) => {
        ...
        if (isHeader) {
            return (<Text bold color={Colors.AccentCyan}>
          {padded}
        </Text>);
        }
        return <Text>{padded}</Text>;
    };
    const renderRow = (cells, isHeader = false) => (<Box flexDirection="row">
      <Text>│ </Text>
      {cells.map((cell, index) => (<React.Fragment key={index}>
    ...
    return (<Box flexDirection="column" marginY={1}>
      ...
      {renderSeparator()}
      {rows.map((row, index) => (<React.Fragment key={index}>{renderRow(row)}</React.Fragment>))}
      {renderBottomBorder()}
    </Box>);
};
