...
const MarkdownDisplayInternal = ({ text, isPending, availableTerminalHeight, terminalWidth, }) => {
    ...
    lines.forEach((line, index) => {
        ...
        if (inCodeBlock) {
            ...
            if (fenceMatch &&
                ...
                contentBlocks.push(<RenderCodeBlock key={key} content={codeBlockContent} lang={codeBlockLang} isPending={isPending} availableTerminalHeight={availableTerminalHeight} terminalWidth={terminalWidth}/>);
                inCodeBlock = false;
                codeBlockContent = [];
                codeBlockLang = null;
                codeBlockFence = '';
        ...
        if (codeFenceMatch) {
            ...
            if (index + 1 < lines.length &&
                ...
                contentBlocks.push(<Box key={key}>
            <Text wrap="wrap">
            ...
            if (tableHeaders.length > 0 && tableRows.length > 0) {
                contentBlocks.push(<RenderTable key={`table-${contentBlocks.length}`} headers={tableHeaders} rows={tableRows} terminalWidth={terminalWidth}/>);
            }
            inTable = false;
            tableRows = [];
            tableHeaders = [];
          ...
          <Text dimColor>---</Text>
            ...
            switch (level) {
                case 1:
                    headerNode = (<Text bold color={Colors.AccentCyan}>
              <RenderInline text={headerText}/>
            </Text>);
                    break;
                case 2:
                    headerNode = (<Text bold color={Colors.AccentBlue}>
              <RenderInline text={headerText}/>
            </Text>);
                    break;
                case 3:
                    headerNode = (<Text bold>
              <RenderInline text={headerText}/>
            </Text>);
                    break;
                case 4:
                    headerNode = (<Text italic color={Colors.Gray}>
              <RenderInline text={headerText}/>
            </Text>);
                    break;
                default:
            ...
            if (headerNode)
                contentBlocks.push(<Box key={key}>{headerNode}</Box>);
        }
        else if (ulMatch) {
            const leadingWhitespace = ulMatch[1];
            const marker = ulMatch[2];
            const itemText = ulMatch[3];
            contentBlocks.push(<RenderListItem key={key} itemText={itemText} type="ul" marker={marker} leadingWhitespace={leadingWhitespace}/>);
        }
        else if (olMatch) {
            const leadingWhitespace = olMatch[1];
            const marker = olMatch[2];
            const itemText = olMatch[3];
            contentBlocks.push(<RenderListItem key={key} itemText={itemText} type="ol" marker={marker} leadingWhitespace={leadingWhitespace}/>);
        }
        else {
            if (line.trim().length === 0) {
    ...
    if (inCodeBlock) {
        contentBlocks.push(<RenderCodeBlock key="line-eof" content={codeBlockContent} lang={codeBlockLang} isPending={isPending} availableTerminalHeight={availableTerminalHeight} terminalWidth={terminalWidth}/>);
    }
...
const RenderInlineInternal = ({ text }) => {
    ...
    while ((match = inlineRegex.exec(text)) !== null) {
        if (match.index > lastIndex) {
            nodes.push(<Text key={`t-${lastIndex}`}>
        ...
        try {
            if (fullMatch.startsWith('**') &&
                ...
                renderedNode = (<Text key={key} bold>
            {fullMatch.slice(BOLD_MARKER_LENGTH, -BOLD_MARKER_LENGTH)}
                ...
                renderedNode = (<Text key={key} italic>
            {fullMatch.slice(ITALIC_MARKER_LENGTH, -ITALIC_MARKER_LENGTH)}
                ...
                renderedNode = (<Text key={key} strikethrough>
            {fullMatch.slice(STRIKETHROUGH_MARKER_LENGTH, -STRIKETHROUGH_MARKER_LENGTH)}
                ...
                if (codeMatch && codeMatch[2]) {
                    renderedNode = (<Text key={key} color={Colors.AccentPurple}>
              {codeMatch[2]}
            </Text>);
                }
                else {
              ...
              <Text color={Colors.AccentBlue}> ({url})</Text>
                ...
                renderedNode = (<Text key={key} underline>
...
const RenderCodeBlockInternal = ({ content, lang, isPending, availableTerminalHeight, terminalWidth, }) => {
    ...
    if (isPending && availableTerminalHeight !== undefined) {
        ...
        if (content.length > MAX_CODE_LINES_WHEN_PENDING) {
            if (MAX_CODE_LINES_WHEN_PENDING < MIN_LINES_FOR_MESSAGE) {
                ...
                return (<Box padding={CODE_BLOCK_PADDING}>
            ...
            return (<Box flexDirection="column" padding={CODE_BLOCK_PADDING}>
    ...
    return (<Box flexDirection="column" padding={CODE_BLOCK_PADDING} width={terminalWidth} flexShrink={0}>
      {colorizedCode}
    </Box>);
};
...
const RenderListItemInternal = ({ itemText, type, marker, leadingWhitespace = '', }) => {
    ...
    return (<Box paddingLeft={indentation + LIST_ITEM_PREFIX_PADDING} flexDirection="row">
...
const RenderTableInternal = ({ headers, rows, terminalWidth, }) => (<TableRenderer headers={headers} rows={rows} terminalWidth={terminalWidth}/>);
