...
describe('handleAtCommand', () => {
    ...
    it('should process a valid text file path', async () => {
        ...
        expect(mockReadManyFilesExecute).toHaveBeenCalledWith({ paths: [filePath], respectGitIgnore: true }, abortController.signal);
    ...
    it('should process a valid directory path and convert to glob', async () => {
        ...
        expect(mockReadManyFilesExecute).toHaveBeenCalledWith({ paths: [resolvedGlob], respectGitIgnore: true }, abortController.signal);
    ...
    it('should correctly unescape paths with escaped spaces', async () => {
        ...
        expect(mockReadManyFilesExecute).toHaveBeenCalledWith({ paths: [unescapedPath], respectGitIgnore: true }, abortController.signal);
    });
    it('should handle multiple @file references', async () => {
        ...
        expect(mockReadManyFilesExecute).toHaveBeenCalledWith({ paths: [file1, file2], respectGitIgnore: true }, abortController.signal);
        expect(result.processedQuery).toEqual([
            { text: `@${file1} @${file2}` },
            { text: '\n--- Content from referenced files ---' },
            { text: `\nContent from @${file1}:\n` },
    ...
    it('should handle a mix of valid, invalid, and lone @ references', async () => {
        ...
        vi.mocked(fsPromises.stat).mockImplementation(async (p) => {
            ...
            if (p.toString().endsWith(file2Glob))
                throw Object.assign(new Error('ENOENT'), { code: 'ENOENT' });
            return { isDirectory: () => false }; // Default
        });
        ...
        expect(mockReadManyFilesExecute).toHaveBeenCalledWith({ paths: [file1, resolvedFile2], respectGitIgnore: true }, abortController.signal);
        expect(result.processedQuery).toEqual([
    ...
    it('should process a file path case-insensitively', async () => {
        ...
        expect(mockAddItem).toHaveBeenCalledWith({ type: 'user', text: query }, 134);
        ...
        { paths: [queryPath], respectGitIgnore: true }, abortController.signal);
        expect(mockAddItem).toHaveBeenCalledWith(expect.objectContaining({
            type: 'tool_group',
            tools: [expect.objectContaining({ status: ToolCallStatus.Success })],
        }), 134);
    ...
    describe('git-aware filtering', () => {
        it('should skip git-ignored files in @ commands', async () => {
            ...
            mockFileDiscoveryService.shouldGitIgnoreFile.mockImplementation((path) => path === gitIgnoredFile);
            const result = await handleAtCommand({
                query,
                config: mockConfig,
                addItem: mockAddItem,
            ...
            expect(mockFileDiscoveryService.shouldGitIgnoreFile).toHaveBeenCalledWith(gitIgnoredFile);
            expect(mockOnDebugMessage).toHaveBeenCalledWith(`Path ${gitIgnoredFile} is git-ignored and will be skipped.`);
            expect(mockOnDebugMessage).toHaveBeenCalledWith('Ignored 1 git-ignored files: node_modules/package.json');
            expect(mockReadManyFilesExecute).not.toHaveBeenCalled();
            expect(result.processedQuery).toEqual([{ text: query }]);
            expect(result.shouldProceed).toBe(true);
        });
        it('should process non-git-ignored files normally', async () => {
            ...
            expect(mockFileDiscoveryService.shouldGitIgnoreFile).toHaveBeenCalledWith(validFile);
            expect(mockReadManyFilesExecute).toHaveBeenCalledWith({ paths: [validFile], respectGitIgnore: true }, abortController.signal);
            expect(result.processedQuery).toEqual([
                { text: `@${validFile}` },
                { text: '\n--- Content from referenced files ---' },
        ...
        it('should handle mixed git-ignored and valid files', async () => {
            ...
            expect(mockFileDiscoveryService.shouldGitIgnoreFile).toHaveBeenCalledWith(gitIgnoredFile);
            expect(mockOnDebugMessage).toHaveBeenCalledWith(`Path ${gitIgnoredFile} is git-ignored and will be skipped.`);
            expect(mockOnDebugMessage).toHaveBeenCalledWith('Ignored 1 git-ignored files: .env');
            expect(mockReadManyFilesExecute).toHaveBeenCalledWith({ paths: [validFile], respectGitIgnore: true }, abortController.signal);
            expect(result.processedQuery).toEqual([
        ...
        it('should always ignore .git directory files', async () => {
            ...
            expect(mockFileDiscoveryService.shouldGitIgnoreFile).toHaveBeenCalledWith(gitFile);
