...
describe('useCompletion git-aware filtering integration', () => {
    ...
    it('should filter git-ignored entries from @ completions', async () => {
        ...
        mockFileDiscoveryService.shouldGitIgnoreFile.mockImplementation((path) => path.includes('dist'));
        const { result } = renderHook(() => useCompletion('@d', testCwd, true, slashCommands, mockConfig));
    ...
    it('should filter git-ignored directories from @ completions', async () => {
        ...
        // Mock git ignore service to ignore certain files
        mockFileDiscoveryService.shouldGitIgnoreFile.mockImplementation((path) => path.includes('node_modules') ||
            path.includes('dist') ||
            path.includes('.env'));
        const { result } = renderHook(() => useCompletion('@', testCwd, true, slashCommands, mockConfig));
        // Wait for async operations to complete
        await act(async () => {
            await new Promise((resolve) => setTimeout(resolve, 150)); // Account for debounce
        });
    ...
    it('should handle recursive search with git-aware filtering', async () => {
        ...
        vi.mocked(fs.readdir).mockImplementation(async (dirPath) => {
            ...
            if (dirPath.endsWith('/temp')) {
                return [{ name: 'temp.log', isDirectory: () => false }];
            }
            return [];
        });
        // Mock git ignore service
        mockFileDiscoveryService.shouldGitIgnoreFile.mockImplementation((path) => path.includes('node_modules') || path.includes('temp'));
        const { result } = renderHook(() => useCompletion('@t', testCwd, true, slashCommands, mockConfig));
        // Wait for async operations to complete
        await act(async () => {
    ...
    it('should not perform recursive search when disabled in config', async () => {
        ...
        renderHook(() => useCompletion('@d', testCwd, true, slashCommands, mockConfigNoRecursive));
        await act(async () => {
            await new Promise((resolve) => setTimeout(resolve, 150));
        });
    ...
    it('should work without config (fallback behavior)', async () => {
        ...
        const { result } = renderHook(() => useCompletion('@', testCwd, true, slashCommands, undefined));
        await act(async () => {
            await new Promise((resolve) => setTimeout(resolve, 150));
        });
    ...
    it('should handle directory-specific completions with git filtering', async () => {
        ...
        mockFileDiscoveryService.shouldGitIgnoreFile.mockImplementation((path) => path.includes('.log'));
        const { result } = renderHook(() => useCompletion('@src/comp', testCwd, true, slashCommands, mockConfig));
        await act(async () => {
            await new Promise((resolve) => setTimeout(resolve, 150));
        });
    ...
    it('should use glob for top-level @ completions when available', async () => {
        ...
        vi.mocked(glob).mockResolvedValue(globResults);
        const { result } = renderHook(() => useCompletion('@s', testCwd, true, slashCommands, mockConfig));
        await act(async () => {
            await new Promise((resolve) => setTimeout(resolve, 150));
        });
...
//# sourceMappingURL=useCompletion.integration.test.js.map