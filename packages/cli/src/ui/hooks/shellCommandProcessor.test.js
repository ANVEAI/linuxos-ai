...
describe('useShellCommandProcessor', () => {
    let spawnEmitter;
    let addItemToHistoryMock;
    let setPendingHistoryItemMock;
    let onExecMock;
    let onDebugMessageMock;
    let configMock;
    let geminiClientMock;
    beforeEach(async () => {
        const { spawn } = await import('child_process');
        spawnEmitter = new EventEmitter();
        spawnEmitter.stdout = new EventEmitter();
        spawnEmitter.stderr = new EventEmitter();
        spawn.mockReturnValue(spawnEmitter);
    ...
    const renderProcessorHook = () => renderHook(() => useShellCommandProcessor(addItemToHistoryMock, setPendingHistoryItemMock, onExecMock, onDebugMessageMock, configMock, geminiClientMock));
    ...
    it('should handle binary output', async () => {
        ...
        act(() => {
            result.current.handleShellCommand('cat myimage.png', abortController.signal);
        });
        expect(onExecMock).toHaveBeenCalledTimes(1);
        const execPromise = onExecMock.mock.calls[0][0];
        act(() => {
    ...
    it('should handle command failure', async () => {
        ...
        const abortController = new AbortController();
        act(() => {
            result.current.handleShellCommand('a-bad-command', abortController.signal);
        });
        const execPromise = onExecMock.mock.calls[0][0];
        act(() => {
...
//# sourceMappingURL=shellCommandProcessor.test.js.map