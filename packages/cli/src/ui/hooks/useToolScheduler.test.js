...
import { ToolConfirmationOutcome, ApprovalMode, // Import from core
...
const mockToolRequiresConfirmation = {
    ...mockTool,
    name: 'mockToolRequiresConfirmation',
    displayName: 'Mock Tool Requires Confirmation',
    shouldConfirmExecute: vi.fn(async () => ({
        type: 'edit',
...
describe('useReactToolScheduler in YOLO Mode', () => {
    ...
    it('should skip confirmation and execute tool directly when yoloMode is true', async () => {
        ...
        expect(mockToolRequiresConfirmation.execute).toHaveBeenCalledWith(request.args, expect.any(AbortSignal), undefined);
...
describe('useReactToolScheduler', () => {
    ...
    it('should schedule and execute a tool call successfully', async () => {
        ...
        expect(mockTool.execute).toHaveBeenCalledWith(request.args, expect.any(AbortSignal), undefined);
        expect(onComplete).toHaveBeenCalledWith([
            expect.objectContaining({
                status: 'success',
                request,
    ...
    it.skip('should throw error if scheduling while already running', async () => {
        mockToolRegistry.getTool.mockReturnValue(mockTool);
        const longExecutePromise = new Promise((resolve) => setTimeout(() => resolve({ llmContent: 'done', returnDisplay: 'done display' }), 50));
