...
import { MCPDiscoveryState, MCPServerStatus, getMCPDiscoveryState, getMCPServerStatus, } from '@google/gemini-cli-core';
...
describe('useSlashCommandProcessor', () => {
    ...
    let mockOpenThemeDialog;
    let mockOpenAuthDialog;
    let mockOpenEditorDialog;
    let mockPerformMemoryRefresh;
    let mockSetQuittingMessages;
    let mockTryCompressChat;
    let mockGeminiClient;
    let mockConfig;
    let mockCorgiMode;
    const mockUseSessionStats = useSessionStats;
    beforeEach(() => {
        mockAddItem = vi.fn();
        ...
        ShowMemoryCommandModule.createShowMemoryAction.mockClear();
    ...
    const getProcessorHook = (showToolDescriptions = false) => {
        ...
        return renderHook(() => useSlashCommandProcessor(mockConfig, settings, [], mockAddItem, mockClearItems, mockLoadHistory, mockRefreshStatic, mockSetShowHelp, mockOnDebugMessage, mockOpenThemeDialog, mockOpenAuthDialog, mockOpenEditorDialog, mockPerformMemoryRefresh, mockCorgiMode, showToolDescriptions, mockSetQuittingMessages));
    };
    const getProcessor = (showToolDescriptions = false) => getProcessorHook(showToolDescriptions).result.current;
    describe('/memory add', () => {
        it('should return tool scheduling info on valid input', async () => {
            ...
            expect(mockAddItem).toHaveBeenNthCalledWith(1, // User message
            expect.objectContaining({
                type: MessageType.USER,
                text: `/memory add ${fact}`,
            }), expect.any(Number));
            expect(mockAddItem).toHaveBeenNthCalledWith(2, // Info message about attempting to save
            expect.objectContaining({
        ...
        it('should show usage error and return true if no text is provided', async () => {
            ...
            expect(mockAddItem).toHaveBeenNthCalledWith(2, // After user message
            expect.objectContaining({
    ...
    describe('/memory show', () => {
        it('should call the showMemoryAction and return true', async () => {
            ...
            vi.mocked(ShowMemoryCommandModule.createShowMemoryAction).mockReturnValue(mockReturnedShowAction);
            const { handleSlashCommand } = getProcessor();
            let commandResult = false;
            await act(async () => {
                commandResult = await handleSlashCommand('/memory show');
            });
            expect(ShowMemoryCommandModule.createShowMemoryAction).toHaveBeenCalledWith(mockConfig, expect.any(Object), expect.any(Function));
    ...
    describe('/stats command', () => {
        it('should show detailed session statistics', async () => {
            ...
            // Assert
            expect(mockAddItem).toHaveBeenNthCalledWith(2, // Called after the user message
            expect.objectContaining({
    ...
    describe('/about command', () => {
        it('should show the about box with all details including auth and project', async () => {
            ...
            const { result } = renderHook(() => useSlashCommandProcessor(mockConfig, settings, [], mockAddItem, mockClearItems, mockLoadHistory, mockRefreshStatic, mockSetShowHelp, mockOnDebugMessage, mockOpenThemeDialog, mockOpenAuthDialog, mockOpenEditorDialog, mockPerformMemoryRefresh, mockCorgiMode, false, mockSetQuittingMessages));
            // Act
            await act(async () => {
    ...
    describe('/bug command', () => {
        ...
        const getExpectedUrl = (description, sandboxEnvVar, seatbeltProfileVar, cliVersion) => {
...
`;
            let url = 'https://github.com/google-gemini/gemini-cli/issues/new?template=bug_report.yml';
            if (description) {
        ...
        it('should call open with the correct GitHub issue URL and return true', async () => {
            ...
            const expectedUrl = getExpectedUrl(bugDescription, process.env.SANDBOX, process.env.SEATBELT_PROFILE, 'test-version');
            let commandResult = false;
            await act(async () => {
        ...
        it('should use the custom bug command URL from config if available', async () => {
            ...
            const bugCommand = {
                urlTemplate: 'https://custom-bug-tracker.com/new?title={title}&info={info}',
            };
            mockConfig = {
                ...mockConfig,
...
`;
    ...
    describe('/quit and /exit commands', () => {
        ...
        it.each([['/quit'], ['/exit']])('should handle %s, set quitting messages, and exit the process', async (command) => {
            const { handleSlashCommand } = getProcessor();
    ...
    describe('/mcp command', () => {
        ...
        it('should display configured MCP servers with status indicators and their tools', async () => {
            ...
            const mockGetToolsByServer = vi.fn().mockImplementation((serverName) => {
                if (serverName === 'server1')
                    return mockServer1Tools;
                if (serverName === 'server2')
                    return mockServer2Tools;
                if (serverName === 'server3')
                    return mockServer3Tools;
                return [];
            });
            mockConfig = {
                ...mockConfig,
        ...
        it('should display tool descriptions when showToolDescriptions is true', async () => {
            ...
            expect(message).toContain('\u001b[1mserver1\u001b[0m - Ready (2 tools)');
            expect(message).toContain('\u001b[32mThis is a server description\u001b[0m');
            // Check that tool descriptions are included (with ANSI color codes)
            expect(message).toContain('\u001b[36mtool1\u001b[0m');
            expect(message).toContain('\u001b[32mThis is tool 1 description\u001b[0m');
            expect(message).toContain('\u001b[36mtool2\u001b[0m');
            expect(message).toContain('\u001b[32mThis is tool 2 description\u001b[0m');
            expect(commandResult).toBe(true);
        });
    ...
    describe('/compress command', () => {
        it('should call tryCompressChat(true)', async () => {
            ...
            mockTryCompressChat.mockImplementationOnce(async (force) => {
