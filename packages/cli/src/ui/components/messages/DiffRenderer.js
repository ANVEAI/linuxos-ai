...
export const DiffRenderer = ({ diffContent, filename, tabWidth = DEFAULT_TAB_WIDTH, availableTerminalHeight, terminalWidth, }) => {
    ...
    if (parsedLines.length === 0) {
        return (<Box borderStyle="round" borderColor={Colors.Gray} padding={1}>
...
const renderDiffContent = (parsedLines, filename, tabWidth = DEFAULT_TAB_WIDTH, availableTerminalHeight, terminalWidth) => {
    ...
    return (<MaxSizedBox maxHeight={availableTerminalHeight} maxWidth={terminalWidth} key={key}>
      {displayableLines.reduce((acc, line, index) => {
            ...
            acc.push(<Box key={lineKey} flexDirection="row">
