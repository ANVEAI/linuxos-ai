...
vi.mock('@google/gemini-cli-core', async () => {
    const actualCore = await vi.importActual('@google/gemini-cli-core');
    return {
        ...actualCore,
        GeminiClient: vi.fn(),
        ToolRegistry: vi.fn(),
...
describe('runNonInteractive', () => {
    ...
    beforeEach(() => {
        ...
        process.stdout.write = mockProcessStdoutWrite; // Use any to bypass strict signature matching for mock
        mockProcessExit = vi
            .fn()
            .mockImplementation((_code) => undefined);
        process.exit = mockProcessExit; // Use any for process.exit mock
    });
    afterEach(() => {
        vi.restoreAllMocks();
    ...
    it('should handle a single tool call and respond', async () => {
        ...
        const { executeToolCall: mockCoreExecuteToolCall } = await import('@google/gemini-cli-core');
        ...
        const stream1 = (async function* () {
            yield { functionCalls: [functionCall] };
        })();
        ...
        expect(mockCoreExecuteToolCall).toHaveBeenCalledWith(mockConfig, expect.objectContaining({ callId: 'fc1', name: 'testTool' }), mockToolRegistry, expect.any(AbortSignal));
        expect(mockChat.sendMessageStream).toHaveBeenLastCalledWith(expect.objectContaining({
    ...
    it('should handle error during tool execution', async () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith('Error executing tool errorTool: Tool execution failed badly');
        expect(mockChat.sendMessageStream).toHaveBeenLastCalledWith(expect.objectContaining({
            message: [errorResponsePart],
        }));
        expect(mockProcessStdoutWrite).toHaveBeenCalledWith('Could not complete request.');
    });
    ...
    it('should not exit if a tool is not found, and should send error back to model', async () => {
        ...
        expect(mockProcessStdoutWrite).toHaveBeenCalledWith('Unfortunately the tool does not exist.');
    });
});
