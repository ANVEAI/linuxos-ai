...
import { EXTENSIONS_CONFIG_FILENAME, EXTENSIONS_DIRECTORY_NAME, loadExtensions, } from './extension.js';
...
describe('loadExtensions', () => {
    ...
    it('should load context file path when GEMINI.md is present', () => {
        const workspaceExtensionsDir = path.join(tempWorkspaceDir, EXTENSIONS_DIRECTORY_NAME);
    ...
    it('should load context file path from the extension config', () => {
        const workspaceExtensionsDir = path.join(tempWorkspaceDir, EXTENSIONS_DIRECTORY_NAME);
        fs.mkdirSync(workspaceExtensionsDir, { recursive: true });
        createExtension(workspaceExtensionsDir, 'ext1', '1.0.0', false, 'my-context-file.md');
        const extensions = loadExtensions(tempWorkspaceDir);
...
function createExtension(extensionsDir, name, version, addContextFile = false, contextFileName) {
    const extDir = path.join(extensionsDir, name);
    fs.mkdirSync(extDir);
    fs.writeFileSync(path.join(extDir, EXTENSIONS_CONFIG_FILENAME), JSON.stringify({ name, version, contextFileName }));
    if (addContextFile) {
