...
SettingScope, } from './settings.js';
const MOCK_WORKSPACE_DIR = '/mock/workspace';
// Use the (mocked) SETTINGS_DIRECTORY_NAME for consistency
const MOCK_WORKSPACE_SETTINGS_PATH = pathActual.join(MOCK_WORKSPACE_DIR, SETTINGS_DIRECTORY_NAME, 'settings.json');
vi.mock('fs');
...
describe('Settings Loading and Merging', () => {
    ...
    describe('loadSettings', () => {
        ...
        it('should handle JSON parsing errors gracefully', () => {
            ...
            const userError = settings.errors.find((e) => e.path === USER_SETTINGS_PATH);
            expect(userError).toBeDefined();
            expect(userError?.message).toBe(userReadError.message);
            const workspaceError = settings.errors.find((e) => e.path === MOCK_WORKSPACE_SETTINGS_PATH);
        ...
        it('should resolve environment variables in workspace settings', () => {
            ...
            expect(settings.workspace.settings.nested.value).toBe('workspace_endpoint_from_env');
        ...
        it('should prioritize workspace env variables over user env variables if keys clash after resolution', () => {
            ...
            const settings = loadSettings(MOCK_WORKSPACE_DIR);
            expect(settings.user.settings.configValue).toBe('user_value_for_user_read');
            expect(settings.workspace.settings.configValue).toBe('workspace_value_for_workspace_read');
        ...
        it('should correctly pass through null, boolean, and number types, and handle undefined properties', () => {
            ...
            expect(settings.user.settings.nestedObj.anotherEnv).toBe('env_string_nested_value');
    ...
    describe('LoadedSettings class', () => {
        it('setValue should update the correct scope and recompute merged settings', () => {
            ...
            expect(fs.writeFileSync).toHaveBeenCalledWith(USER_SETTINGS_PATH, JSON.stringify({ theme: 'matrix' }, null, 2), 'utf-8');
            loadedSettings.setValue(SettingScope.Workspace, 'contextFileName', 'MY_AGENTS.md');
            expect(loadedSettings.workspace.settings.contextFileName).toBe('MY_AGENTS.md');
            ...
            expect(fs.writeFileSync).toHaveBeenCalledWith(MOCK_WORKSPACE_SETTINGS_PATH, JSON.stringify({ contextFileName: 'MY_AGENTS.md' }, null, 2), 'utf-8');
