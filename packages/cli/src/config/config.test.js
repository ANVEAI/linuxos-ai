...
vi.mock('@google/gemini-cli-core', async () => {
    ...
    return {
        ...
        loadServerHierarchicalMemory: vi.fn((cwd, debug, fileService, extensionPaths) => Promise.resolve({
...
describe('loadCliConfig', () => {
    ...
    it('should set showMemoryUsage to false by default from settings if CLI flag is not present', async () => {
        process.argv = ['node', 'script.js'];
        const settings = { showMemoryUsage: false };
        const config = await loadCliConfig(settings, [], 'test-session');
        expect(config.getShowMemoryUsage()).toBe(false);
    });
...
describe('loadCliConfig telemetry', () => {
    ...
    it('should use telemetry log prompts from settings if CLI flag is not present', async () => {
        process.argv = ['node', 'script.js'];
        const settings = { telemetry: { logPrompts: false } };
        const config = await loadCliConfig(settings, [], 'test-session');
...
describe('mergeExcludeTools', () => {
    it('should merge excludeTools from settings and extensions', async () => {
        const settings = { excludeTools: ['tool1', 'tool2'] };
        const extensions = [
            {
                config: {
                    name: 'ext1',
    ...
    it('should handle overlapping excludeTools between extensions', async () => {
        const settings = { excludeTools: ['tool1'] };
        const extensions = [
            {
                config: {
                    name: 'ext1',
