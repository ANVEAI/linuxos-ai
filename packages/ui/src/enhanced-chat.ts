...
export class EnhancedChatInterface {
  private router: SystemAdminRouter;
...
╰─────────────────────────────────────────────────────────────────╯
  ...
  private async showSuggestions(health: SystemHealth) {
  ...
  async handleSystemCommand(input: string) {
  ...
  private displayCommandPreview(command: SystemCommand) {
  ...
  async askConfirmation(command: SystemCommand): Promise<boolean> {
    console.log('\n' + boxen(
  ...
  private async executeSystemCommand(command: SystemCommand) {
  ...
  private displayCommandResult(result: any) {
  ...
  private showHelp() {
    ...
    helpTable.push(
      ['install <package>', 'Install software packages', 'install nginx'],
      ['install oracle', 'Install Oracle Database', 'install oracle with 8GB memory'],
      ['setup webserver', 'Setup web server stack', 'setup nginx with SSL'],
      ['clean system', 'Clean temporary files', 'clean system aggressively'],
      ['secure system', 'Run security audit', 'secure my server'],
      ['analyze performance', 'Check system performance', 'why is my system slow?'],
      ['organize files', 'Organize file structure', 'organize my downloads folder'],
      ['status', 'Show system status', 'status'],
      ['help', 'Show this help', 'help'],
      ['clear', 'Clear screen', 'clear'],
      ['exit/quit', 'Exit LinuxOS-AI', 'exit']
    );
  ...
  private getCpuDescription(cpu: number): string {
    if (cpu > 80) return 'High usage - check processes';
    if (cpu > 60) return 'Moderate usage';
